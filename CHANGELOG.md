...
### 추가
- `parsers/cohere_parse.py` → `_cohere.md`. `COHERE_API_KEY` 필요(트라이얼 키 월 1,000콜 무료, 이후 $1.50/1,000쪽). SDK 없이 urllib로 `/v2/parse` REST 직접 호출 — 새 pip 의존성 없음.
- 마크다운 + HTML 표 + 이미지 설명(`<!-- 이미지 id (category): 설명 -->` 주석). 한국어가 9개 안정 지원 언어에 포함.

### 편입 판단 근거
- **넣은 이유**: 한국어 안정 지원 + 쪽당 단가가 우리 클라우드 파서 대역에서 가장 낮은 축(장당 약 2원, corepin과 동급) + 공급사 벤치마크상 Mistral OCR 4보다 위. Mistral과 같은 슬롯(대용량 완전성·교차검증)의 경쟁자다.
- **넣되 등급을 비운 이유**: 공급사 벤치마크는 근거가 아니다. `parser-eval` 능력 매트릭스에는 형식 가능 여부만 채웠고(텍스트 PDF·스캔 PDF·이미지 가능, HWPX·DOCX·XLSX 불가), 등급은 첫 평가에서 매긴다.

### 구조적 제약 (다른 클라우드 파서와 다른 점)
- 엔드포인트가 **이미지만** 받는다 → PDF는 PyMuPDF로 렌더해 **쪽당 1회 호출**. 쪽수 = 호출 수 = 과금 단위(gvision과 같은 경유 구조).
- 쪽당 출력 상한 8,192 토큰. API가 절단 신호를 주지 않아 출력 토큰 8,000 이상인 쪽을 경고로 표시한다.
- confidence 미제공(gvision과 달리 저신뢰 판독 표적을 만들지 못함).
- 입력 상한(20MB·50MP) 초과 시 렌더 배율을 자동으로 낮춘다.

### 실행 계약 (2026-08-31 계약 준수)
- 쪽 호출 429·5xx는 지수 백오프 2회 재시도(총 3회), 그래도 실패한 쪽이 하나라도 있으면 출력 미작성 + 종료 코드 1. 전 쪽 공백도 동일. 실행 시작 시 이전 출력 제거. 비-TTY 안전.

### 검증
- 오프라인 회귀 29종 + 재시도 분기 3종(429 재시도 후 성공·401 즉시 실패·500 3회 후 오류) 통과.
- **실호출 1회 스모크 테스트 통과**(한국어 합성 1쪽: 제목·소제목·괘선 표 3행·목록·숫자). 문서에 적힌 응답 스키마(`pages[].markdown.content`)가 실물과 일치했고, 한글 제목이 `#`/`##` 헤딩으로, 표가 `<table><thead>/<tbody>`로, 숫자 2·4·13이 정확히 나왔다. 가운뎃점(`·`)도 보존.
- ⚠️ **이것은 스모크 테스트이지 평가가 아니다.** 합성 1쪽으로는 완전성·장문 거동·스캔 판독을 말할 수 없으므로 특성 비교표의 등급은 `parser-eval` 평가 전까지 「미평가」로 남는다.
...
- **장부 소급 3건 연결**: 규칙만 있고 장부 판정이 없던 세 절(ODL 본문 숫자 검증 게이트, 인쇄체 인구통계 교차표, 인쇄용 책자 연속 표·병합셀)을 `parser-eval` 장부에 소급 기입(`20260514-odl-numeric-dropout`·`20260705-demographic-crosstab`·`20260828-print-booklet-merged-cells`, 9행)하고 각 절의 `*근거:*` 줄에 eval_id·doc.id·등급을 병기. 연속 표 절의 `n≈3`은 장부에 문서 1건만 있어 `n=1`로 정정.
