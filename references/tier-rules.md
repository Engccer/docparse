...
- **판정 방법: PUA 코드포인트를 센다.** 육안 대조가 아니라 기계 지표다. 파서 출력마다 PUA 덩어리 수와 LaTeX 토큰 수를 세어 나란히 놓는다.
  ...
  **절대 개수가 아니라 밀도로 가른다.** 한컴 PDF는 수식이 없어도 원문자·기호를 PUA로 흘리는 경우가 있어(ODL 출력 3건에서 14·102·225 덩어리 관측) "PUA > 0"만으로는 오판한다. 본문 1만 자당 PUA 코드포인트로 재면 갈린다.

  | 문서 | 1만 자당 PUA |
  |---|---:|
  | 수식 없는 한컴 PDF(ODL, 2건) | 6.3 / 8.2 |
