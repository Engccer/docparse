...
핵심 기준은 "결정론이냐 통계냐"가 아니라 **그 문서가 애초에 추론을 요구하는가**다. 원본이 명시적일 때(그려진 격자 + 임베딩된 글자) 추론은 순수한 하방 위험이고, 원본이 모호할 때(스캔·수기·괘선 없는 표)는 추론이 유일한 해법이라 기존 다중 교차검증이 정확히 옳다. 이 게이트는 기존 파이프라인의 대체재가 아니라 그 아래에 빠져 있던 바닥 층이다.
