...
위 "Gemini + Upstage + ODL" 권장 조합은 **객관식 국·영·사 시험지 전제**다. 수식·변수가 본문에 들어간 시험지(대학 구술고사 수리·자연계열, 수학·과학 문항지)에서는 **ODL과 Upstage가 수식 글리프를 통째로 버린다**. 한글 워드프로세서로 조판된 시험지는 수식이 `HancomEQN` 같은 전용 심볼 글꼴의 CID로 들어가 있어 텍스트 레이어 직접 추출이 빈 문자열을 반환하기 때문이다. 결과는 조용한 훼손이라 더 위험하다.

- 실측(2026-08-29, 서울대 78쪽 면접·구술고사 기출): 원문 `자연수 n의 값이 3이고 정수 k의 값이 0, 2, 4일 때`
  - LlamaParse v2: `자연수 $n$의 값이 3이고 정수 $k$의 값이 0, 2, 4 일 때` (정상, LaTeX)
  - Upstage: `자연수  의 값이  이고 정수  의 값이 , ,  일 때` (변수·수치 전멸)
  - ODL: 위와 동일하게 전멸
- **판정 방법**: 파싱 후 수학 섹션에서 변수 한 개를 골라 세 파서를 대조한다. 조사(`의`, `가`)만 남고 피수식어가 빈 칸이면 그 파서는 그 문서에서 Primary 부적격이다.
- **역할 분담**: 수식·본문은 LlamaParse v2 Primary, **해설 표의 행 구조는 Upstage가 정확**하다(LlamaParse는 쪽 경계에서 표를 끊고 첫 열 라벨 셀을 병합한다). 라벨 행 개수를 Upstage 기준으로 세어 Primary의 누락을 찾는 것이 값싼 게이트다.
