...
- **small (≤15p)**: Gemini 단독 (OCR 내장). **단, 표에 숫자 데이터가 있는 스캔 양식은 "스캔+표 숫자" 규칙(아래)을 따른다(Gemini 단독 불가)**.
...
- **Mistral**: 수기 오기 대량 무단 교정 + 수기 한글 이름 9/22 인식 → 수기 문서 3자 교차 가치도 낮음(부적합).
- **Gemini**: 체크박스를 페이지 단위로 뒤집음(사용 금지), 흘림체 본문 붕괴.
