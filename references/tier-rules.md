...
**승격 규칙**: PASS면 그대로 채택(`_fused_v3_pdfplumber.md`, Step 5~7 생략 가능). 경고(셀 불일치·미배정 단어·병합 의심 셀·표 미검출)가 하나라도 뜨면 파서가 **출력 파일 자체를 만들지 않으므로**(실패본이 작업 폴더에 남아 퓨전에 잘못 편입되는 것 방지) 기본 티어 표로 승격한다. 실패 시 안전하게 떨어지므로 기존 동작을 해치지 않는다.
