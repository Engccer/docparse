...
- **판정 방법: PUA 코드포인트를 센다.** 육안 대조가 아니라 기계 지표다. 파서 출력마다 PUA 덩어리 수와 LaTeX 토큰 수를 세어 나란히 놓는다.
  ...
  같은 78쪽 문서 실측: PUA 덩어리가 LlamaParse 0 / Upstage 731 / ODL 646(코드포인트로는 0 / 1,227 / 1,275), LaTeX 토큰은 450 / 0 / 0. **수식이 없는 대조 문서에서는 세 파서 모두 0~14로 떨어지므로 변별이 선다.** PUA가 세 자리인데 LaTeX이 0이면 그 파서는 그 문서에서 Primary 부적격이다.
