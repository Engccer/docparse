...
CLOUD = [
    ...
    {"name": "openai", "modules": [], "pip": "pymupdf",
     "keys": ["OPENAI_API_KEY"], "url": "https://platform.openai.com/api-keys",
     "note": "범용 멀티모달(기본 gpt-5.6-terra). 전용 OCR 엔드포인트 없음. 호출은 표준 라이브러리. ⚠ PDF 기본 경로(쪽 구간 분할)만 PyMuPDF 필요 — 이미지·Office·--pages-per-call 0은 불필요"},
